...
CREATE TABLE IF NOT EXISTS CustomerOrderDetail (
    -- Only care about quantity (no price included)
...
CREATE TABLE IF NOT EXISTS LocationBin (
    ...
    Section VARCHAR(50),
    Shelf VARCHAR(50),
    Capacity INT NOT NULL CHECK (Capacity > 0) -- THE FIXED CAPACITY - DO NOT CHANGE
);
CREATE TABLE IF NOT EXISTS Stock (
    ...
    LocID INT NOT NULL,
    Quantity INT NOT NULL CHECK (Quantity >= 0),
    -- get updated by trigger in Stock Transaction
...
CREATE TABLE IF NOT EXISTS PickingList (
    ...
    -- the moment the list is generated
...
CREATE TABLE IF NOT EXISTS OrderTransaction (
    ...
    CustomerOrderID INT NOT NULL,
    RefID INT,
    -- Can be ShipmentID or PickListID ?? Enforced by triggerr?
    TransactionTime TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
DECLARE NewProductID,
    NewLocID INT;
SELECT ProductID,
    LocID INTO NewProductID,
    NewLocID
FROM Stock
