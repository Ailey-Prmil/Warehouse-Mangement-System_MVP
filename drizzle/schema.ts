...
export const customerOrderDetail = mysqlTable(
  "CustomerOrderDetail",
  {
    orderDetailId: int("OrderDetailID").autoincrement().notNull(),
    customerOrderId: int("CustomerOrderID")
      .notNull()
      .references(() => customerOrder.customerOrderId, { onDelete: "cascade" }),
...
export const inboundShipment = mysqlTable(
  "InboundShipment",
  {
    shipId: int("ShipID").autoincrement().notNull(),
    shipmentTime: timestamp("ShipmentTime", { mode: "string" }).defaultNow(),
  },
  (table) => [
    primaryKey({ columns: [table.shipId], name: "InboundShipment_ShipID" }),
  ]
);

export const inboundShipmentDetail = mysqlTable(
  "InboundShipmentDetail",
  {
    detailId: int("DetailID").autoincrement().notNull(),
    shipId: int("ShipID")
      .notNull()
      .references(() => inboundShipment.shipId, { onDelete: "cascade" }),
    productId: int("ProductID")
      .notNull()
      .references(() => product.productId, { onDelete: "cascade" }),
...
export const inspection = mysqlTable(
  "Inspection",
  {
    ...
    stockId: int("StockID")
      .notNull()
      .references(() => stock.stockId, { onDelete: "cascade" }),
    inspectDate: timestamp("InspectDate", { mode: "string" }).defaultNow(),
...
export const locationBin = mysqlTable(
  "LocationBin",
  {
    locId: int("LocID").notNull(),
...
export const orderTransaction = mysqlTable(
  "OrderTransaction",
  {
...
export const outboundShipment = mysqlTable(
  "OutboundShipment",
  {
    shipmentId: int("ShipmentID").autoincrement().notNull(),
    shipmentDate: timestamp("ShipmentDate", { mode: "string" }).defaultNow(),
...
export const pickingList = mysqlTable(
  "PickingList",
  {
...
export const purchaseOrderDetail = mysqlTable(
  "PurchaseOrderDetail",
  {
    poDetailId: int("PODetailID").autoincrement().notNull(),
    poId: int("PO_ID")
      .notNull()
      .references(() => purchaseOrder.poId, { onDelete: "cascade" }),
    productId: int("ProductID")
      .notNull()
      .references(() => product.productId, { onDelete: "cascade" }),
...
export const stock = mysqlTable(
  "Stock",
  {
    ...
    locId: int("LocID")
      .notNull()
      .references(() => locationBin.locId, { onDelete: "cascade" }),
    quantity: int("Quantity").notNull(),
...
export const stockTransaction = mysqlTable(
  "StockTransaction",
  {
    transactionId: int("TransactionID").autoincrement().notNull(),
    productId: int("ProductID")
